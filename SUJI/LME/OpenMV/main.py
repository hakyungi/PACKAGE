...
#                   5 : ?
#
#         Data Type : ( 1일때,  2나 3일때)
# DataToSend[2] : 첫번째 값 (TOF1, X)
# DataToSend[3] : 두번째 값 (TOF2, Y)
# DataToSend[4] : 세번째 값 (TOF3, r)
# DataToSend[5] : 네번째 값 (yaw, r)
# DataToSend[6] : 다섯번째 값 (roll, 사용안함)
# DataToSend[7] : 여섯번째 값 (pitch, 사용안함)
#----------------------------------------------------------------------------------
...
lpf2 = LPF2.EV3_LPF2(1, 'P1', 'P0', modes, 85, timer = 4, freq = 5) #ID 85 to match our block
lpf2.initialize()
...
def send_SensorValue():
    ...
    # PACKET 만들어 보내기
    #-------------------------------------------------------------
    DataToSend = [PacketCount, 1, tof1V, tof2V, tof3V, int(imuV[0]*10), int(imuV[1]*10), int(imuV[2]*10)]
    ...
    #        print(c)
        ...
        if Circles:
            ...
            for c in Circles:
                ...
                if hist.get_threshold().value() < 15 :
                    ...
                    if not BlackCircle:
                        ...
                        if c.r() > BlackCircle.r():
                            BlackCircle = c
                elif hist.get_threshold().value() > 40 and hist.get_threshold().value() < 100:
                    PacketCount+=1
            ...
            if BlackCircle:
                ...
                DataToSend = [PacketCount, 2, -2, -2, 0, 0, 0, 0]
            lpf2.load_payload('Int16',DataToSend)
            ...
            if SilverCircle:
                ...
                DataToSend = [PacketCount, 3, -3, -3, 0, 0, 0, 0]
            lpf2.load_payload('Int16',DataToSend)
