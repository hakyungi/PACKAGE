...
while(True):
    ...
    temp1E = TEMP1.getEnvCelsius()
    temp1O = TEMP1.getObjCelsius()

    temp2E = TEMP2.getEnvCelsius()
    temp2O = TEMP2.getObjCelsius()

    imuV = IMU.euler()

    print("TOF1 : ", tof1V)             #측정거리 mm
    print("TOF2 : ", tof2V)
    print("TOF3 : ", tof3V)
    print("TOF4 : ", tof4V)
    print("주변 온도 1 %s C"% temp1E)    #섭씨 주변온도
    print("타겟 온도 1 %s C"% temp1O)    #섭씨 대상온도
    print("주변 온도 2 %s C"% temp2E)    #섭씨 주변온도
    print("타겟 온도 2 %s C"% temp2O)    #섭씨 대상온도
    print("IMU   : ", imuV)             #YAW, ROLL, PITCH
...
#        print('BLACK  : %d' %len(BlackCircles))
    ...
    addData('uInt16', tof4V)        # TOF4 : 부호 없는 2 바이트 정수('<H').
    addData('float', temp1E)        # 주변온도 : 4 바이트 부동 소수('f')
    addData('float', temp1O)        # 대상온도 : 4 바이트 부동 소수('f')
    addData('float', temp2E)        # 주변온도 : 4 바이트 부동 소수('f')
    addData('float', temp2O)        # 대상온도 : 4 바이트 부동 소수('f')
